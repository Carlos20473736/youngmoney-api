...
try {
    ...
    while ($row = $result->fetch_assoc()) {
        // Mascarar o email: esconder os últimos 2 caracteres antes do @
        $email = $row['email'];
        $emailParts = explode('@', $email);
        $localPart = $emailParts[0];
        $domain = $emailParts[1];
        
        // Se o email tiver mais de 2 caracteres antes do @, esconde os últimos 2
        if (strlen($localPart) > 2) {
            $visiblePart = substr($localPart, 0, -2);
            $maskedEmail = $visiblePart . '**@' . $domain;
        } else {
            // Se tiver 2 ou menos, mostra apenas o primeiro caractere
